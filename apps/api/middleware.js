 ...
 * preflight before any cross-origin POST/PATCH/DELETE. This middleware returns
 * the correct preflight response and injects CORS headers into all other responses.
 *
 * Allowed origins are read from the ALLOWED_ORIGINS environment variable — see
 * lib/cors.js. Origins not in the list receive no allow-origin header and are
 * blocked by the browser's same-origin policy.
 *
 * The SSE stream route sets its own CORS headers directly on its streaming
 * Response, since middleware cannot reliably modify headers on a response that
 * has already started streaming. It uses the same buildCorsHeaders utility to
